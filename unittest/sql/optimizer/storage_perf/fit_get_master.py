...
for mode in modes:
    ...
    prep_cmd[2] = f'get.{mode}.res'
    prep_cmd[4] = f'get.{mode_name}.prep'
    run_cmd(' '.join(prep_cmd))
    for col_count in col_counts:
        prep_cmd[4] = f'get.{mode_name}.prep.{str(col_count)}'
...
for mode in modes:
    mode_name = mode_names[mode]
    fit_cmd[2] = f'get.{mode_name}.prep'
    fit_cmd[4] = f'get.{mode_name}.model'
    run_cmd(' '.join(fit_cmd))
    for col_count in col_counts:
        apply_cmd[2] = f'get.{mode_name}.prep.{str(col_count)}'
        apply_cmd[4] = f'get.{mode_name}.fit.{str(col_count)}'
        apply_cmd[6] = f'get.{mode_name}.model'
